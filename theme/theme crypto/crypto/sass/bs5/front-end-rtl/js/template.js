...
(function($) {
    ...
    **/
    var launch = new Date(2023, 05, 14, 09, 00);
    /**
    * The script
    **/
    ...
    function setDate(){
        var now = new Date();
        if( launch < now ){
            days.html('<h1>0</H1><p>روز</p>');
            hours.html('<h1>0</h1><p>ساعت</p>');
            minutes.html('<h1>0</h1><p>دقیقه</p>');
            seconds.html('<h1>0</h1><p>ثانیه</p>');
        }
        else{
            var s = -now.getTimezoneOffset()*60 + (launch.getTime() - now.getTime())/1000;
            var d = Math.floor(s/86400);
            days.html('<h1>'+d+'</h1><p>روز'+(d>1?'':''),'</p>');
            s -= d*86400;

            var h = Math.floor(s/3600);
            hours.html('<h1>'+h+'</h1><p>ساعت'+(h>1?'':''),'</p>');
            s -= h*3600;

            var m = Math.floor(s/60);
            minutes.html('<h1>'+m+'</h1><p>دقیقه'+(m>1?'':''),'</p>');

            s = Math.floor(s-m*60);
            seconds.html('<h1>'+s+'</h1><p>ثانیه'+(s>1?'':''),'</p>');
...
var options = {
	title: 'این وب سایت از کوکی ها استفاده می کند.<br> مقررات عمومی حفاظت از داده ها (GDPR) و دستورالعمل حفظ حریم خصوصی الکترونیکی (ePR) بر نحوه استفاده شما به عنوان مالک وب سایت و ردیابی آنلاین بازدیدکنندگان تأثیر می گذارد.',
	message: 'ما از کوکی ها برای شخصی سازی محتوا و تبلیغات، ارائه ویژگی های رسانه های اجتماعی و تجزیه و تحلیل ترافیک خود استفاده می کنیم. ما همچنین اطلاعات مربوط به استفاده شما از سایت خود را با شرکای رسانه های اجتماعی، تبلیغات و تجزیه و تحلیل خود به اشتراک می گذاریم که ممکن است آن را با سایر اطلاعاتی که به آنها ارائه کرده اید یا از استفاده شما از خدماتشان جمع آوری کرده اند، ترکیب کنند.',
	delay: 600,
	expires: 1,
	link: '#privacy',
	onAccept: function(){
		var myPreferences = $.fn.ihavecookies.cookie();
		console.log('بله  تنظیمات برگزیده زیر ذخیره شد...');
		console.log(myPreferences);
	},
	uncheckBoxes: true,
	acceptBtnLabel: 'پذیرش کوکی ها',
	moreInfoLabel: '',
	cookieTypesTitle: 'انتخاب کنید که کدام کوکی ها را می خواهید بپذیرید',
	fixedCookieTypeLabel: 'ضروری ',
	fixedCookieTypeDesc: 'این موارد برای کارکرد صحیح وب سایت ضروری هستند.'
}
...
$(document).ready(function() {
	...
	if ($.fn.ihavecookies.preference('marketing') === true) {
		console.log('این مورد باید اجرا شود زیرا بازاریابی پذیرفته شده است.');
	}
